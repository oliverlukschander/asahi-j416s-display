#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0140 experiment; never reload/reboot.

Config-only, same appledrm.ko/thunderbolt.ko as 0139 (its plane/hotplug
diagnostics stay active). Drops usb4_protocol_probe=1 and
usb4_native_dpin=1 from the module options. dcp_usb4_native_route()
(dcp.c) returns true whenever both are set, and
apple_probe_typec_ports() (apple_drv.c) uses that to restrict the right
port's connector's encoder possible_crtcs to ONLY dcpext1's CRTC bit --
a leftover assumption from the pre-0127 "native DPIN0" single-purpose
experiment, still being set by every candidate's module options since
0129 without being revisited. This directly conflicts with
usb4_route_prefer_fixed_diag=1 (kept, unchanged), which forces the
software tunnel route onto dcpext0: dcpext0's CRTC is never in
possible_crtcs, so every atomic commit attempt from Hyprland fails the
generic DRM encoder/crtc pairing check before ever reaching a
driver-level check -- confirmed directly: dcp_hotplug's new 0139
diagnostic showed crtc=0000000000000000 (NULL) for the tunnel connector,
and apple_plane_atomic_check() was never once called for its plane
across a full boot+replug, while a live Hyprland replug test showed
Aquamarine actively retrying a cascade of fallback modes, each rejected
with EINVAL. Every other usb4_native_dpin call site (grepped exhaustively)
is a dev_info_once() diagnostic print with no other effect; safe to drop.
Requires 0139's reconnect fix and diagnostics to already be in place
(checked below); does not redo them.
"""
import argparse
import hashlib
import json
...
CONFIG = Path('/etc/modprobe.d/j416s-0140-dpin0-mode-guess.conf')
PRIOR_CONFIG = Path('/etc/modprobe.d/j416s-0139-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0140-before')
PRIOR_CONFIG_BACKUP = BACKUP / 'prior-0139.conf'
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0140-initramfs-') as tmp:
        ...
        if prior_cfg.exists():
            raise RuntimeError('Superseded 0139 conf still in initramfs; remove it first')
...
def restore():
    ...
    os.sync()
    print('Restored pre-0140 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    if not PRIOR_CONFIG.is_file():
        raise RuntimeError('Expected 0139 conf missing; nothing to supersede')
    ...
    try:
        ...
        restore()
        raise
    print('0140 installed and initramfs verified. No reboot performed.', flush=True)

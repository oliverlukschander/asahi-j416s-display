#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0116 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0116-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0116-before')
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0116-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0116 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0116 installed and initramfs verified. No reboot performed.', flush=True)

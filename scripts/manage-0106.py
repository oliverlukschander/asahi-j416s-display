#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0106 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0106-dp-video-counter.conf')
BACKUP = Path('/var/tmp/j416s-0106-before')
...
CANDIDATES = {
    ...
    'thunderbolt': (ROOT / 'src/thunderbolt/thunderbolt.ko',
                   'a70debca0d7fd8a53d8807358a6db922049f321ed24b8a6415ebf4f75b3a8898'),
}
TARGETS = {
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0106-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0106 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0106 installed and initramfs verified. Hub and direct external display MUST remain unplugged. No reboot performed.', flush=True)

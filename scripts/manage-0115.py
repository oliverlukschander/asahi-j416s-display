#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0115 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0115-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0115-before')
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0115-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0115 modules and initramfs; no live reload/reboot.', flush=True)

#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0099 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0099-native-dpin.conf')
BACKUP = Path('/var/tmp/j416s-0099-before')
...
CANDIDATES = {
    ...
    'appledrm': (ROOT / 'src/appledrm/appledrm.ko',
                '65ae9be244ba6475ff1d65ae2db069167ebbddd0edaa994bb798753d88173514'),
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0099-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0099 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0099 installed and initramfs verified. Hub MUST remain unplugged. No reboot performed.', flush=True)

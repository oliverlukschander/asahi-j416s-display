#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0129 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0129-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0129-before')
...
CANDIDATES = {
    ...
    'appledrm': (ROOT / 'src/appledrm/appledrm.ko',
                '131f3d85cad626e5387df05b55016f80dac60196cb5d555c4dc73503ab96a6a9'),
...
def preflight():
    ...
    # Same as 0128: the hub may stay connected across install/reboot/disarm --
    # this action is file-only (module copy, initramfs rebuild), never live
    # MMIO or module reload, so an attached hub carries no risk here. The
    # external-display connector check below is kept: it is a different,
    # still-useful signal (catches a route that already came up).
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0129-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0129 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0129 installed and initramfs verified. No reboot performed.', flush=True)

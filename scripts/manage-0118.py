#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0118 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0118-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0118-before')
...
CANDIDATES = {
    ...
    'appledrm': (ROOT / 'src/appledrm/appledrm.ko',
                '69ef18690a67d277ffaa0eac63bc3126fd3e2b41cdfb0e109373ec72122b4469'),
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0118-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0118 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0118 installed and initramfs verified. No reboot performed.', flush=True)

#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0102 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0102-native-dpin.conf')
BACKUP = Path('/var/tmp/j416s-0102-before')
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0102-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0102 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0102 installed and initramfs verified. Hub and direct external display MUST remain unplugged. No reboot performed.', flush=True)

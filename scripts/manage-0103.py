#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0103 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0103-native-dpin.conf')
BACKUP = Path('/var/tmp/j416s-0103-before')
...
CANDIDATES = {
    ...
    'mux': (ROOT / 'src/mux/mux-apple-display-crossbar.ko',
            '5ae53864377909684d735afe0a4c7b04303e5b38a99d3c15ce25142a47ce5e32'),
    'appledrm': (ROOT / 'src/appledrm/appledrm.ko',
                '9defc2ff3ba5b6a36e15097516ed20ed3e6a92bfe2a4bf8fe7b943c58b81fd09'),
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0103-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0103 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0103 installed and initramfs verified. Hub and direct external display MUST remain unplugged. No reboot performed.', flush=True)

#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0138 experiment; never reload/reboot.

Same module set as 0137 except thunderbolt.ko: tb_dp_activate()'s
"if (ret) return ret;" on the deactivate path let a departing dpout
port's deterministic -ENODEV short-circuit (tb.h's
"if (port->sw->is_unplugged) return -ENODEV") skip
ops->dp_tunnel_deactivate() -- which apple.c relies on to clear its own
apple_dpin_ctx "alive"/"handed" latch. Without it, a physical
unplug/replug reforms the Thunderbolt tunnel but never re-arms DCP's own
connect flow. Changed to "if (ret && active) return ret;" (both spots);
the caller (tb_tunnel_deactivate()) already discards the return value on
the deactivate path, so the activate path is byte-for-byte unchanged.
Requires 0137's linkcfg_completion fix to already be in place (checked
...
CONFIG = Path('/etc/modprobe.d/j416s-0138-dpin0-mode-guess.conf')
PRIOR_CONFIG = Path('/etc/modprobe.d/j416s-0137-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0138-before')
PRIOR_CONFIG_BACKUP = BACKUP / 'prior-0137.conf'
...
CANDIDATES = {
    ...
    'appledrm': (ROOT / 'src/appledrm/appledrm.ko',
                'a8f4c4b5d1a8451316149b8bc9599a514f9c00b7c1132ab8780b38590adc275f'),
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0138-initramfs-') as tmp:
        ...
        if prior_cfg.exists():
            raise RuntimeError('Superseded 0137 conf still in initramfs; remove it first')
...
def restore():
    ...
    os.sync()
    print('Restored pre-0138 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    if not PRIOR_CONFIG.is_file():
        raise RuntimeError('Expected 0137 conf missing; nothing to supersede')
    ...
    try:
        ...
        restore()
        raise
    print('0138 installed and initramfs verified. No reboot performed.', flush=True)

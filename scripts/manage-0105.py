#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0105 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0105-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0105 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0105 installed and initramfs verified. Hub and direct external display MUST remain unplugged. No reboot performed.', flush=True)

#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0137 experiment; never reload/reboot.

Same module set as 0135/0136 except appledrm.ko: dptxep.c's
SET_ACTIVE_LANE_COUNT handler no longer gates linkcfg_completion behind
dcp_usb4_drm_allowed() (usb4_force_dptx), a dead flag left over from a
manual-training sysfs knob removed in 0dc9f50 -- it now completes
unconditionally for lane_count > 0, matching the hardware-validated
aurora-silicon/linux#8 reference exactly. Requires 0136's stale-config
cleanup to already be in place (checked below); does not redo it.
"""
import argparse
import hashlib
import json
...
CONFIG = Path('/etc/modprobe.d/j416s-0137-dpin0-mode-guess.conf')
PRIOR_CONFIG = Path('/etc/modprobe.d/j416s-0135-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0137-before')
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0137-initramfs-') as tmp:
        ...
        if prior_cfg.exists():
            raise RuntimeError('Superseded 0135 conf still in initramfs; remove it first')
...
def restore():
    ...
    os.sync()
    print('Restored pre-0137 modules and initramfs; no live reload/reboot.', flush=True)


PRIOR_CONFIG_BACKUP = BACKUP / 'prior-0135.conf'
...
def install():
    ...
    if not PRIOR_CONFIG.is_file():
        raise RuntimeError('Expected 0135 conf missing; nothing to supersede')
    ...
    try:
        ...
        restore()
        raise
    print('0137 installed and initramfs verified. No reboot performed.', flush=True)

#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0130 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0130-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0130-before')
...
def preflight():
    ...
    # Same as 0128/0129: the hub may stay connected across install/reboot/
    # disarm -- this action is file-only (module copy, initramfs rebuild),
    # never live MMIO or module reload, so an attached hub carries no risk
    # here. The external-display connector check below is kept: it is a
    # different, still-useful signal (catches a route that already came up).
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0130-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0130 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0130 installed and initramfs verified. No reboot performed.', flush=True)

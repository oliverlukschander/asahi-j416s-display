#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0101 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0101-native-dpin.conf')
BACKUP = Path('/var/tmp/j416s-0101-before')
...
CANDIDATES = {
    'atc': (ROOT / 'src/phy/phy-apple-atc.ko',
            '794b47de6e3b1faab8da51532ca1e264da0cb8eb9513188e2a5ff5ae0b4d86b4'),
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0101-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0101 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0101 installed and initramfs verified. Hub and direct external display MUST remain unplugged. No reboot performed.', flush=True)

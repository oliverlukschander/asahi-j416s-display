#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0120 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0120-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0120-before')
...
CANDIDATES = {
    ...
    'appledrm': (ROOT / 'src/appledrm/appledrm.ko',
                'b401f2913e55206c7b2dc2249e804c29d92e19b5af35baca6f1f1d91f00d9cf5'),
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0120-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0120 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0120 installed and initramfs verified. No reboot performed.', flush=True)

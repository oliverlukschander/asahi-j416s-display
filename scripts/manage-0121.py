#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0121 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0121-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0121-before')
...
CANDIDATES = {
    ...
    'appledrm': (ROOT / 'src/appledrm/appledrm.ko',
                '15dd9b2346879b3b771200a45362ad81e281fde158d3fae7632369073c5b6ca8'),
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0121-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0121 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0121 installed and initramfs verified. No reboot performed.', flush=True)

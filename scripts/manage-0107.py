#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0107 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0107-dp-video-counter.conf')
BACKUP = Path('/var/tmp/j416s-0107-before')
OPTIONS = ('options appledrm usb4_protocol_probe=1 usb4_native_dpin=1 usb4_tunnel_clock=1\n'
           ...
           'options thunderbolt dp_video_counter=1\n')
CANDIDATES = {
    'atc': (ROOT / 'src/phy/phy-apple-atc.ko',
            'e47f03cef54c44c816c85a7565f41cde046a9a364b9db9857653c5fbaad0b0c9'),
    'mux': (ROOT / 'src/mux/mux-apple-display-crossbar.ko',
            '38e0756e986d236eddb458e2480f62f45eed1b3c2baeb5e61aa2e55a522f8b24'),
    ...
    'thunderbolt': (ROOT / 'src/thunderbolt/thunderbolt.ko',
                   'ebbd7a80568be7422d403bc6d05a5d0a577dbf24d4f939c359e0561f69255906'),
}
TARGETS = {
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0107-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0107 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0107 installed and initramfs verified. Hub and direct external display MUST remain unplugged. No reboot performed.', flush=True)

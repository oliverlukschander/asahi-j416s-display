#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0131 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0131-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0131-before')
...
CANDIDATES = {
    ...
    'thunderbolt_apple': (ROOT / 'src/thunderbolt/thunderbolt_apple.ko',
                        '4bd921e908a491ddc3ccd2dfd701fb39ae015df2824f0ad9862ca8ca71ef314d'),
    'thunderbolt': (ROOT / 'src/thunderbolt/thunderbolt.ko',
                   'fd5f7196144fc760459f71cac094d19901c554531e96ab6c2c68096c7e9b7465'),
}
TARGETS = {
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0131-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0131 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0131 installed and initramfs verified. No reboot performed.', flush=True)

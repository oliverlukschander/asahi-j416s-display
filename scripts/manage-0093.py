#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0093 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0093-native-dpin.conf')
BACKUP = Path('/var/tmp/j416s-0093-before')
...
CANDIDATES = {
    'appledrm': (ROOT / 'src/appledrm/appledrm.ko',
                '2e5cb7146f07aa7f23bfb4f03afd2c5e66c0e3efea81b68fc49fdfc2dafdcefc'),
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0093-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0093 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0093 installed and initramfs verified. Hub MUST remain unplugged. No reboot performed.', flush=True)

#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0141 experiment; never reload/reboot.

Diagnostic only, no behavior change beyond 0140 (which is kept: dropped
usb4_protocol_probe/usb4_native_dpin, unchanged here). 0140 was installed
and tested -- it did NOT fix the symptom: a live replug produced the
...
before 0140), and apple_plane_atomic_check() (0139's instrumentation)
still never fires for the tunnel connector's plane. So either the
possible_crtcs theory was wrong, or something else excludes dcpext0's
CRTC bit besides the exclusion 0140 removed.

Added two more diagnostics to settle it directly: (1) in
apple_probe_typec_ports() (apple_drv.c), log each candidate DCP's
dcp-index/native_route/has_candidate/crtc_mask contribution and the
final computed possible_crtcs mask for the port, at probe time -- this
shows definitively whether dcpext0's CRTC bit is actually present in the
mask now; (2) in dcp_crtc_atomic_check() (dcp.c), log every reach of
this per-CRTC hook (crtc id, mode, active, enable) -- this shows whether
the generic DRM core even gets as far as a per-CRTC driver hook for
dcpext0's CRTC during a failed commit, or rejects the whole transaction
earlier (encoder/CRTC pairing) before any driver code for that CRTC runs
at all. Requires 0140 to already be in place (checked below); does not
redo it.
"""
import argparse
import hashlib
...
CONFIG = Path('/etc/modprobe.d/j416s-0141-dpin0-mode-guess.conf')
PRIOR_CONFIG = Path('/etc/modprobe.d/j416s-0140-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0141-before')
PRIOR_CONFIG_BACKUP = BACKUP / 'prior-0140.conf'
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0141-initramfs-') as tmp:
        ...
        if prior_cfg.exists():
            raise RuntimeError('Superseded 0140 conf still in initramfs; remove it first')
...
def restore():
    ...
    os.sync()
    print('Restored pre-0141 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    if not PRIOR_CONFIG.is_file():
        raise RuntimeError('Expected 0140 conf missing; nothing to supersede')
    ...
    try:
        ...
        restore()
        raise
    print('0141 installed and initramfs verified. No reboot performed.', flush=True)

#!/usr/bin/env python3
"""Stage, disarm or restore the bounded 0132 experiment; never reload/reboot."""
import argparse
import hashlib
import json
import os
...
CONFIG = Path('/etc/modprobe.d/j416s-0132-dpin0-mode-guess.conf')
BACKUP = Path('/var/tmp/j416s-0132-before')
...
CANDIDATES = {
    ...
    'thunderbolt_apple': (ROOT / 'src/thunderbolt/thunderbolt_apple.ko',
                        '91caddc7f37594c6d326b01562656f12d99c8e0b67adc2bde84d3df75645368b'),
...
def verify_image(armed):
    with tempfile.TemporaryDirectory(prefix='j416s-0132-initramfs-') as tmp:
...
def restore():
    ...
    os.sync()
    print('Restored pre-0132 modules and initramfs; no live reload/reboot.', flush=True)
...
def install():
    ...
    try:
        ...
        restore()
        raise
    print('0132 installed and initramfs verified. No reboot performed.', flush=True)
